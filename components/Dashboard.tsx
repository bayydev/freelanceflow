            ...
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="bg-cyber-panel border border-cyber-border rounded-xl p-6">
                ...
                <form onSubmit={handleAddQuickTask} className="flex gap-2 mb-4">
                  <input type="text" value={newTaskTitle} onChange={(e) => setNewTaskTitle(e.target.value)} placeholder="Ex: Pagar internet..." className="flex-1 bg-cyber-dark border border-cyber-border rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-cyber-primary" />
                  <button type="submit" className="bg-slate-800 text-slate-200 p-2 rounded-lg hover:bg-slate-700"><Plus size={18} /></button>
                </form>
                <ul className="space-y-2">
                  {tasks.length === 0 && <li className="text-xs text-slate-600 text-center py-4">Caixa de entrada vazia.</li>}
                  {tasks.map(task => (
